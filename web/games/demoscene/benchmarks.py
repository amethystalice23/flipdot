...
def generate():
    ...
    starttime = time.time()
    global point1
    point1 = list((Point(random.random() * 800, random.random() * 600) for _ in range(amount)))
    print("creating point1 in %f s" % (time.time()-starttime))
    starttime = time.time()
    global point2
    point2 = list(((random.random() * 800, random.random() * 600) for _ in range(amount)))
    print("creating point2 in %f s" % (time.time()-starttime))
    starttime = time.time()
    global point3
    point3 = list((array.array("f", (random.random() * 800, random.random() * 600)) for _ in range(amount)))
    print("creating point3 in %f s" % (time.time()-starttime))
    starttime = time.time()
    global point4
    point4 = list((Point2(random.random() * 800, random.random() * 600) for _ in range(amount)))
    print("creating point4 in %f s" % (time.time()-starttime))
    starttime = time.time()
    global point5
    point5 = list(([random.random() * 800, random.random() * 600] for _ in range(amount)))
    print("creating point5 in %f s" % (time.time()-starttime))
    starttime = time.time()
    global point6
    point6 = list(({"x" : random.random() * 800, "y" : random.random() * 600} for _ in range(amount)))
    print("creating point6 in %f s" % (time.time()-starttime))
    starttime = time.time()
    global point7
    point7 = list((Point2(random.random() * 800, random.random() * 600) for _ in range(amount)))
    print("creating point7 in %f s" % (time.time()-starttime))
    global point8
    point8 = list((PointC(random.random() * 800, random.random() * 600) for _ in range(amount)))
...
def calculate2a(*args, **kwds):
    list(map(lambda a: (a[0] + 1.0, a[1] + 1.0), point2))
